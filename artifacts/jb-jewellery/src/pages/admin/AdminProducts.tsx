...
export default function AdminProducts() {
  ...
  const handleSave = () => {
    ...
    if (isNew) {
      const newP: Product = { ...editing, id: `prod-${Date.now()}` } as Product;
      setProds(prev => [...prev, newP]);
    } else {
      setProds(prev => prev.map(p => p.id === editing.id ? { ...editing } as Product : p));
    }
    setShowForm(false);
  };
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  {['Product', 'Category', 'MRP', 'Price', 'Discount', 'Reviews', 'Tags', 'Actions'].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {prods.map(p => (
                  <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-gray-500">⭐ {p.rating} ({p.reviews})</td>
                    <td className="px-4 py-3">
